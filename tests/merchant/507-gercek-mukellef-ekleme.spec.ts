 ...
 await kozanOption.click();
  ...
  await telNoInput1.fill(uretilenTelNo);
    ...
    await telNoInput2.fill(uretilenTelNo);
     ...
     try {
      const denemeRows = page.getByRole('row').filter({ hasText: /DENEME/ });
      const denemeCount = await denemeRows.count();
      
      if (denemeCount > 0) {
        // Ana Bayi sütunu boş olan DENEME cihazlarını filtrele
        const uyeIsyeriDenemeRows: any[] = [];
        
        for (let i = 0; i < denemeCount; i++) {
          const row = denemeRows.nth(i);
          ...
          if (!anaBayiText || anaBayiText.trim() === '') {
            uyeIsyeriDenemeRows.push(row);
          }
        }
        
        if (uyeIsyeriDenemeRows.length > 0) {
          const randomIndex = Math.floor(Math.random() * uyeIsyeriDenemeRows.length);
          const denemeRow = uyeIsyeriDenemeRows[randomIndex];
          await denemeRow.getByRole('checkbox').check();
          console.log(`✅ DENEME üye işyeri seçildi.`);
        } else {
          console.log('❌ DENEME üye işyeri bulunamadı.');
        }
      } else {
        console.log('❌ DENEME ile başlayan üye işyeri bulunamadı.');
      }
    } catch (error) {
      console.log('❌ DENEME ile başlayan üye işyeri bulunamadı');
    }
