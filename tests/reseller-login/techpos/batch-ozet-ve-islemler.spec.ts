...
test('TechPOS - Batch Özet ve İşlemler (reseller-login)', async ({ page }) => {
  
    console.log('===>  TechPOS Batch Özet ve İşlemler (reseller-login)  <===');
    ...
    // 25 gün öncesinin tarihini konsola yazdır
    ...
    console.log(`📅 25 gün öncesi: ${yirmiGunOncesiString}`);
    ...
    // Menü yazımı sistemde ikon ile olabilir; önce ikonlu metni, sonra düz metni dene
    const batchLinkWithIcon = page.getByRole('link', { name: ' Techpos Batch' });
    if (await batchLinkWithIcon.count()) {
        await batchLinkWithIcon.click();
    } else {
